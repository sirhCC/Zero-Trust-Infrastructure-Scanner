...
describe('CLI output and fail-on behavior', () => {
  ...
  const outFile = path.resolve(__dirname, '..', 'examples', 'test-data', 'cli-output.json');
  ...
  it('emits JSON to a file when --output json --out-file is used', () => {
    ...
    const res = spawnSync(
      process.execPath,
      [distCli, 'network', '--output', 'json', '--out-file', outFile, '--scan-depth', '1'],
      { encoding: 'utf8' }
    );
  ...
  it('fails with exit code 1 when --fail-on low (since there are always some findings)', () => {
    ...
    const res = spawnSync(
      process.execPath,
      [distCli, 'network', '--fail-on', 'low', '--scan-depth', '1'],
      { encoding: 'utf8' }
    );
