...
describe('CLI output and fail-on behavior (supply-chain)', () => {
  ...
  const outFile = path.resolve(__dirname, '..', 'examples', 'test-data', 'cli-supply-output.json');
  ...
  it('emits JSON to a file when --output json --out-file is used', () => {
    ...
    const res = spawnSync(
      process.execPath,
      [distCli, 'supply-chain', '--output', 'json', '--out-file', outFile],
      { encoding: 'utf8' }
    );
  ...
  it('fails with exit code 1 when --fail-on low (since there are always some findings)', () => {
    if (!fs.existsSync(distCli)) return; // skip if not built
    const res = spawnSync(process.execPath, [distCli, 'supply-chain', '--fail-on', 'low'], {
      encoding: 'utf8',
    });
