  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const targets = options.targets ? options.targets.split(',').map((t: string) => t.trim()) : ['localhost'];
        const webhooks = options.webhooks ? options.webhooks.split(',').map((w: string) => w.trim()) : [];
        const emailAlerts = options.emailAlerts ? options.emailAlerts.split(',').map((e: string) => e.trim()) : [];
        const monitorConfig = {
          ...
          targets: targets.map((target: string) => ({
            ...
            scan_target: {
              ...
              target: target,
              options: { cloud_provider: null, k8s_namespace: null, policy_file: null, scan_depth: 3 }
            },
          ...
          alerting: {
            enabled: true,
            channels: [
              ...webhooks.map((url: string) => ({ type: 'webhook' as const, config: { url }, enabled: true })),
              ...(options.slackWebhook ? [{ type: 'slack' as const, config: { webhook_url: options.slackWebhook }, enabled: true }] : []),
              ...(options.teamsWebhook ? [{ type: 'teams' as const, config: { webhook_url: options.teamsWebhook }, enabled: true }] : []),
              ...(emailAlerts.length > 0 ? [{ type: 'email' as const, config: { recipients: emailAlerts, smtp: { host: 'localhost', port: 587, secure: false, auth: { user: '', pass: '' } } }, enabled: true }] : [])
            ],
            severity_threshold: 'medium' as const,
            rate_limiting: { max_alerts_per_minute: 5, cooldown_period: 300 }
          },
          websocket: { port: parseInt(options.port), path: '/ws', authentication: Boolean(options.wsToken), token: options.wsToken, max_connections: 100 },
          change_detection: { enabled: true, delta_threshold: 10, baseline_update_frequency: 24, ignore_transient_changes: true }
        } as const;
        ...
        const statusServer = httpStatus.createServer((req: any, res: any) => {
          ...
          const securityHeaders: Record<string, string> = {
            'X-Content-Type-Options': 'nosniff', 'X-Frame-Options': 'DENY', 'Referrer-Policy': 'no-referrer', 'Cache-Control': 'no-store, no-cache, must-revalidate'
          };
          try {
            if (req.method === 'OPTIONS') {
              res.writeHead(204, { 'Access-Control-Allow-Origin': allowOrigin, 'Access-Control-Allow-Methods': allowMethods, 'Access-Control-Allow-Headers': allowHeaders });
              res.end();
              return;
            }
            ...
            if (url.pathname === '/api/status') {
              ...
              res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache', 'Access-Control-Allow-Origin': allowOrigin, ...securityHeaders });
          ...
          try {
            ...
            if (url.pathname === '/api/status') {
              try {
                ...
                res.writeHead((resp as any).status, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'X-Frame-Options': 'DENY', 'Content-Security-Policy': "default-src 'none'" });
                res.end(body);
              } catch (e) {
            ...
            res.writeHead(200, { 'Content-Type': 'text/html', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'X-Frame-Options': 'DENY', 'Content-Security-Policy': csp, 'Permissions-Policy': 'geolocation=(), microphone=(), camera=()' });
